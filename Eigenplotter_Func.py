...
def GetLists():   
    ...
    yaw_list = np.radians(fixList(newDict.get("Ahrs1_bYawRate")))
    ...
    pitchrate_list = np.radians(fixList(newDict.get("Ahrs1_bPitchRate")))
    return time_list,velocity_list,altitude_list,alpha_list, pitch_list,roll_list,yaw_list,aileron_list,rudder_list, elevator_list,rollrate_list,pitchrate_list, eigenmotions


def getEigenmotions():
    lengths = [150.,30.,30.,30.,20.,160.]
    modes = ["phugoid", "short period", "dutch roll","dutch roll yd", "aperiodic roll", "spiral" ]
    time_list,velocity_list,altitude_list,alpha_list, pitch_list,roll_list,yaw_list,aileron_list,rudder_list, elevator_list,rollrate_list,pitchrate_list, eigenmotions=GetLists()
    times=[]
    velocities=[]
    altitudes=[]
    alphas=[]
    ...
    for n in range(len(modes)):
        ...
        pitchrates.append(pitchrate_list[indices[0]:indices[-1]])
    return indices, times, altitudes, velocities, alphas,  pitches, rolls, yaws, ailerons, rudders, elevators, rollrates, pitchrates
